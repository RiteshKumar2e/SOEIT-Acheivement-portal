...
const VerifyAchievementsPage = () => {
    ...
    const handleVerify = async () => {
        if (!action) { toast.error('Please select Approve or Reject'); return; }
        if (action === 'rejected' && !remarks.trim()) { toast.error('Please provide a reason for rejection'); return; }
        setVerifying(true);
        try {
            await adminAPI.verify(selected._id, { action, remarks });
            toast.success(`Achievement ${action === 'approved' ? 'Approved' : 'Rejected'}`);
            setSelected(null);
            setAction('');
            setRemarks('');
            load();
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="page-header" style={{ marginBottom: '2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                    <h2 className="heading-display">Verify Achievements</h2>
                    <p className="page-subtitle">Review and verify achievements submitted by students.</p>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', padding: '0.875rem 1.5rem', background: 'var(--primary-50)', borderRadius: '14px', border: '1px solid var(--primary-100)', boxShadow: 'var(--shadow-xs)' }}>
                    ...
                    <span style={{ fontSize: '0.9rem', color: 'var(--brand-800)', fontWeight: 900, letterSpacing: '0.02em' }}>{total} RECORDS FOUND</span>
                </div>
            </div>
                ...
                <div style={{ minWidth: 0 }}>
                    {loading ? (
                        ...
                        <div className="card" style={{ padding: '6rem 2rem', textAlign: 'center', background: 'var(--slate-50)', border: '2px dashed var(--border-primary)' }}>
                            ...
                            <h3 style={{ fontWeight: 900, color: 'var(--text-primary)', marginBottom: '0.75rem', fontSize: '1.5rem' }}>No pending requests</h3>
                            <p style={{ color: 'var(--text-muted)', fontWeight: 600, maxWidth: '400px', margin: '0 auto' }}>All submitted achievements have been reviewed.</p>
                        </div>
                    ) : (
                ...
                {selected && (
                    <div className="card animate-slide-in" style={{ position: 'sticky', top: '24px', alignSelf: 'start', height: 'calc(100vh - 48px)', display: 'flex', flexDirection: 'column', border: '1px solid var(--border-primary)', borderRadius: '20px', boxShadow: 'var(--shadow-xl)', overflow: 'hidden' }}>
                        <div className="card-header" style={{ padding: '1.75rem', borderBottom: '1px solid var(--border-primary)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--slate-50)' }}>
                            <div>
                                <h4 style={{ margin: 0, fontWeight: 900, fontSize: '1.25rem', letterSpacing: '-0.02em' }}>Verify Details</h4>
                                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>ID: {selected._id.slice(-12).toUpperCase()}</span>
                            </div>
                        ...
                        <div className="card-body" style={{ padding: '1.75rem', overflowY: 'auto', flex: 1 }}>
                            <div style={{ marginBottom: '2rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
                                    ...
                                    <div style={{ padding: '0.4rem 0.875rem', background: 'var(--slate-100)', color: 'var(--text-secondary)', borderRadius: '10px', fontSize: '0.75rem', fontWeight: 900, textTransform: 'uppercase' }}>{selected.level}</div>
                                </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1rem', padding: '0 0.5rem' }}>
                                    {[
                                        { label: 'Student Name', val: selected.student?.name },
                                        { label: 'Department', val: selected.student?.department },
                                        { label: 'Date', val: selected.date ? format(new Date(selected.date), 'dd MMMM yyyy') : 'N/A' },
                                        { label: 'Institution', val: selected.institution || 'Self-Published' }
                                    ].map(({ label, val }) => (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                        {selected.certificateUrl && (
                                            <a href={selected.certificateUrl.startsWith('http') ? selected.certificateUrl : `${STATIC_BASE_URL}${selected.certificateUrl}`} target="_blank" rel="noreferrer"
                                                ...
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    <div style={{ fontSize: '0.9rem', fontWeight: 900, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Certificate</div>
                                                    <div style={{ fontSize: '0.7rem', color: 'var(--brand-500)', fontWeight: 800, textTransform: 'uppercase', marginTop: '0.2rem' }}>Certificate File</div>
                                                </div>
                                                <Eye size={18} />
                                            </a>
                                        )}
                                        {selected.proofFiles?.map((f, i) => (
                                            <a key={i} href={f.url?.startsWith('http') ? f.url : `${STATIC_BASE_URL}${f.url}`} target="_blank" rel="noreferrer"
                                                ...
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    ...
                                                    <div style={{ fontSize: '0.7rem', color: 'var(--brand-500)', fontWeight: 800, textTransform: 'uppercase', marginTop: '0.2rem' }}>Proof Document</div>
                                                </div>
                            ...
                            <div style={{ marginTop: 'auto', paddingTop: '2rem', borderTop: '2px dashed var(--border-primary)' }}>
                                <div style={{ fontSize: '0.8rem', fontWeight: 900, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '1.25rem' }}>Action</div>
                                ...
                                <div className="form-group" style={{ marginBottom: '1.75rem' }}>
                                    <label className="form-label" style={{ fontWeight: 900, fontSize: '0.85rem' }}>Remarks / Reason {action === 'rejected' && <span className="text-danger">*</span>}</label>
                                    <textarea className="form-control" style={{ borderRadius: '14px', border: '1px solid var(--border-primary)', padding: '1rem', fontWeight: 600, fontSize: '0.95rem' }} rows={4} placeholder={action === 'rejected' ? 'Enter reason for rejection...' : 'Add remarks (optional)...'} value={remarks} onChange={e => setRemarks(e.target.value)} />
                                </div>
                                ...
                                <button className="btn btn-primary w-full" style={{ height: '56px', borderRadius: '16px', fontWeight: 900, fontSize: '1.1rem', letterSpacing: '-0.02em', boxShadow: 'var(--shadow-lg)' }} onClick={handleVerify} disabled={!action || verifying}>
                                    {verifying ? <div className="spinner-sm" /> : selected.status === 'pending' ? 'Confirm Decision' : 'Update Status'}
                                </button>
                            </div>
                        </div>
                    </div>
